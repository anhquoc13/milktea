...
class User {
	protected static  $tblname = "websiteperson";
	...
	function find_user($id="",$user_name=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tblname." 
			WHERE U_id = {$id} OR username = '{$user_name}'");
	...
	static function userAuthentication($username,$h_pass){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `websiteperson` WHERE `username` = '". $username ."' and `pass` = '". $h_pass ."'");
		$cur = $mydb->executeQuery();
		if($cur==false){
			die(mysql_error());
		}
